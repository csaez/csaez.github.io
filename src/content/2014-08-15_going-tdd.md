...
Encapsulate your dependencies, _write testable code is up to you_!
...
place (i.e. write all the calls to the filesystem in a particular module,
the DCC calls on other and so on).
...
where your dependencies are, otherwise you won't be able to "replace"
...
mechanism to systematicaly run them (a testing framework), it sounds obvious
but it's not.
...
Boom! We have decided to interface with maya nodes through a class called Rig
and access the library through a couple of functions (there's probably another
class managing json-files/sqllite/any-kind-of-storage under the hood).
Just think about how you would like to interact with your project and write
...
1- Reading the readme snippet I would go writting a test as follow:
    ...
    def setup():
        pose_lib.add_category('test')

    def tear_down():
        pose_lib.remove_category('test')

    @test_with(setup, tear_down)
    def test_list_poses()
        assert pose_lib.list_poses('test') == list()
...
4- ... and refactor the code until all tests passes...
...
there's a way to patch objects and simulate return values and so on).
With no-so-much effort you should be able to run your tests outside
...
There's much more to be said about testing and TDD, things like coverage
and what not... but I think this is enough for a first contact.


What do you think? Is it doable on the projects you are working on?
Do you have any experience working like this? Anything to share?
